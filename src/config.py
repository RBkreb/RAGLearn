...
Rules:
1. Answer solely based on the retrieved context. Do not use any external knowledge.
2. If the context contains sufficient information, answer concisely (1-3 sentences).
3. If the context is insufficient or irrelevant, Just say Can Not Answer then stop.
4. Never speculate, guess, or fabricate.
5. If the context is about a different person/place/event than what was asked, treat it as insufficient.
'''
#When you finished answering,you MUST say:  Task completed'''

@dataclass
class HyDEConfig(LlmConfig):
    model_name:str="qwen3.5-4b"
    base_url:str="http://127.0.0.1:1234/v1"
    api_key:str="nokey"
    temperature=0.75
