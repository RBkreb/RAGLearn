# src/middleware/hooks.py
"""LangChain Middleware Hooks - 占位符实现"""

from langchain.agents.middleware import AgentMiddleware, ModelRequest, ToolCallRequest, AgentState
...
class MiddlewareHooks(AgentMiddleware):
    ...
    def wrap_model_call(self, request: ModelRequest, handler) -> None:
        """Model 调用包装器"""
        print("[Hook] wrap_model_call triggered")
        return handler(request)

    def wrap_tool_call(self, request: ToolCallRequest, handler) -> None:
        """Tool 调用包装器"""
